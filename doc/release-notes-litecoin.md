Ovato Core version *0.15.1* is now available from:

  <https://download.ovato.org/ovato-0.15.1/>
...
A number of changes to the way Ovato Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 ovatos per kB (0.00001 OVO). This is to prepare for dropping the minimum transaction fee to 0.1 ovatos per kB (0.0001 OVO) in the next release.
