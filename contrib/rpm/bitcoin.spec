...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 11002
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 11003
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 11004
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 11005
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 11002
	%{_sbindir}/semanage port -d -p tcp 11003
	%{_sbindir}/semanage port -d -p tcp 11004
	%{_sbindir}/semanage port -d -p tcp 11005
	for selinuxvariant in %{selinux_variants}; do
